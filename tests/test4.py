...
class Test4(TestBase):
    ...
    @mock.patch("buddysdk.settings.Settings")
    def test_register_device_us(self, settings_mock):
        settings_mock.return_value = buddysdk.settings.Settings(TestBase.US_app_id, TestBase.US_app_key)
    ...
    def test_register_device_eu(self, settings_mock):
        settings_mock.return_value = buddysdk.settings.Settings(TestBase.EU_app_id, TestBase.EU_app_key)
        self.setup_with_bad_tokens(settings_mock.return_value)
    ...
    @mock.patch.object(buddysdk.https.Https, "_Https__handle_dictionary_request")
    @mock.patch.object(buddysdk.https.Https, "_Https__hardware_info_file_name", new_callable=mock.PropertyMock)
    def test_hardware_info(self, hardware_info_file_name_mock, handle_dictionary_request_mock):
        handle_dictionary_request_mock.return_value = {buddysdk.https.Https.exception_name: Exception()}
        ...
        settings = buddysdk.settings.Settings(TestBase.US_app_id, TestBase.US_app_key)
